...
| 已确认决策 | DeepSeek 兼容 API 起步；结构化字段为 name + quantity + unit；重复单号提示后允许确认保存；MVP 只保证当前输入不丢；语音不作为硬验收；数据库增加 express_record_items 明细表 |
...
| Excel/CSV 导出 | 按日期、关键词等筛选条件导出入库记录，Excel 可直接打开 |
...
| 视频上传解析生成 Excel | 视频画面用于自动识别快递单号，视频音频用于识别用户口述的物品内容，先做技术 Spike |
...
| 决策项 | 结论 |
| --- | --- |
| LLM 供应商 | 默认使用 DeepSeek OpenAI 兼容 API，后续可通过环境变量切换 Qwen/OpenAI/自建兼容接口 |
| LLM 输出字段 | MVP 使用 `name + quantity + unit`，暂不强制 `remark` |
| 重复单号 | 不静默重复；发现重复时提示用户，可查看原记录、仍然保存为新记录或取消 |
| 离线能力 | MVP 只保证当前输入不丢和可手动重试；完整 IndexedDB 队列、自动同步放 P1 |
| 语音能力 | 语音是增强能力，不作为 MVP 硬验收；文字录入是主链路 |
| 数据库 | MVP 增加 `express_record_items` 明细表，支持后续按物品查询和统计 |
...
| GET | /api/express/export.csv | 按筛选条件导出 CSV，Excel 可直接打开 |
...
### 12.1 视频上传解析 Excel（P1 Spike）

该能力的目标不是从视频画面里识别包裹内物品，而是把视频拆成“画面识别单号 + 音频识别口述内容”两条链路。

推荐流程：

1. 用户上传 10-30 秒短视频。
2. 后端每秒抽取 2-3 帧，优先用条码/二维码识别快递单号。
3. 如果条码识别失败，再对关键帧做 OCR，识别面单上的快递单号文本。
4. 对多帧识别结果做投票，取出现次数最多且格式可信的单号。
5. 使用 ffmpeg 从视频中抽取音频。
6. ASR 将用户口述转换为文本。
7. LLM 将口述文本解析为 `name + quantity + unit` 结构化清单。
8. 前端展示“单号 + 物品清单”预览，由用户确认。
9. 用户可选择入库或导出 CSV/Excel。

P1 Spike 验收标准：

- 能上传一段短视频并完成服务端处理。
- 能从清晰面单画面中识别至少一个候选快递单号。
- 能从视频音频中得到口述文本。
- 能生成与当前入库记录一致的结构化清单。
- 识别失败时保留可编辑的单号输入框和口述文本。
...
- 统计看板
- CSV/Excel 导出
- 视频上传解析生成 Excel（P1 Spike：上传视频 -> 抽帧识别条码/OCR 单号 -> 抽音频 -> ASR 识别口述内容 -> LLM 结构化 -> 人工确认 -> 导出）
- 多账号、多仓库
- 与电商平台订单数据对接
- 私有化部署包
- 付费订阅或一次性部署服务
